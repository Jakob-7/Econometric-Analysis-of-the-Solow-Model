...
edu_data <- as.data.table(read.csv('API_SE.XPD.TOTL.GD.ZS_DS2_en_csv_v2_76269.csv', skip = 4))
...
edu_data <- melt(edu_data, id.vars = c("Country.Name"), measure.vars = patterns("^X"), variable.name = "Year", value.name = "education")
...
edu_data[, Year := as.integer(sub("^X", "", Year))]

# Merge the datasets
data <- merge(gdp_data, saving_data, by = c("Country.Name", "Year"))
data <- merge(data, pop_data, by = c("Country.Name", "Year"))
data <- merge(data, edu_data, by = c("Country.Name", "Year"))

# Check NAs by country
na_count_country <- data[, .(na_gdp = sum(is.na(GDP_per_capita)),
                             na_saving = sum(is.na(savings_rate)),
                             na_growth = sum(is.na(population)),
                             na_edu = sum(is.na(education))),
                         by = Country.Name]
na_count_country
...
data <- data[Country.Name %in% na_count_country[na_gdp < threshold & na_saving < threshold & na_growth < threshold & na_edu < threshold, Country.Name]]

# Check NAs by year
na_count_year <- data[, .(na_gdp = sum(is.na(GDP_per_capita)),
                          na_saving = sum(is.na(savings_rate)),
                          na_growth = sum(is.na(population)),
                          na_edu = sum(is.na(education))),
                      by = Year]
...
data <- data[Year %in% na_count_year[na_gdp < threshold & na_saving < threshold & na_growth < threshold & na_edu < threshold, Year]]
...
data[, education := ifelse(is.na(education), mean(education, na.rm = TRUE), education), by = Country.Name]

# Removing negative values since we are taking logs
data <- data[GDP_per_capita > 0 & savings_rate > 0 & population > 0 & education > 0]

# Running the updated regression with education as an additional regressor
regression_updated <- lm(log(GDP_per_capita) ~ log(savings_rate) + log(population) + log(education), data = data)
summary(regression_updated)
